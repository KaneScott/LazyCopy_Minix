...
PUBLIC int do_copy() {
	...
	/*used to mask 31 bit*/
	int r, c, parentsFD, childsFD, existingLinks;
	struct filp *fil_ptrp, *fil_ptrc, *fil_test;
	struct inode *rip, *testNode, *sharedNode;
	register int oflags;
	const unsigned int SETMSB = 0x80000000;
	const unsigned int MASKMSB = 0x7FFFFFFF;
	unsigned int testVal;
	unsigned char testArray[1000];
	block_t b;
	short scale;
        mode_t bits;
	struct buf *bp;
	...
	rip = alloc_inode(fil_ptrp->filp_ino->i_dev, R_BIT | W_BIT);
	...
	for(c = 0; c < V2_NR_TZONES - 1; c = c + 1) {
		...
		/*printf("CHILD ZONE %d: %d\tPARENT ZONE %d: %d\n", c, rip->i_zone[c], c, fil_ptrp->filp_ino->i_zone[c]);*/
		if(rip->i_zone[c] != NO_ZONE){
			testVal = rip->i_zone[c];
			printf("Whole value: %d\n", testVal );
			printf("Last bit value: %d\n", (testVal >> 31));
			testVal = testVal | SETMSB;
			printf("Last bit value: %d\n", (testVal >> 31));
			printf("Whole value: %d\n", testVal);
			printf("Whole value: %d\n", (testVal & MASKMSB));
			fil_ptrp->filp_ino->i_zone[c] = testVal;
			rip->i_zone[c] = testVal;
		}
		/*printf("IZONE 1 AFTER: %d\n", rip->i_zone[c]);*/
	}
	...
	if (rip->i_zone[9] == NO_ZONE) {
		...
		zero_block (bp);
		printf("Allocated triple indirect zone to program: %d\n", rip->i_zone[9]);
	} else {
		/* Retreive existing data block for pointer manipulation */
		printf("Retreiving the zone[9] from inode: %d\n", rip->i_zone[9]);
		b = (block_t) rip->i_zone[9] << scale;
		bp = get_block(rip->i_dev, b, NORMAL);
	}
	...
	if (fil_ptrp->filp_ino->i_zone[9] == NO_ZONE) {
		...
		while(sharedNode->i_zone[9] != NO_ZONE && sharedNode->i_zone[9] > 0){
			existingLinks = existingLinks + 1;
			memcpy(sharedNode, &(bp->b_data[existingLinks * sizeof(struct inode)]), sizeof(struct inode));
		}
		printf("Existing links: %d\n", existingLinks);
	}


	/*printf("Test block 1 content(child) /wo mask: %d\n", rip->i_zone[0]);
	printf("Test block 1 content(child) /w mask: %d\n", (rip->i_zone[0] & MASKMSB));

	printf("Test block 1 content(parent) /wo mask: %d\n", fil_ptrp->filp_ino->i_zone[0]);
	printf("Test block 1 content(parent) /w mask: %d\n", (fil_ptrp->filp_ino->i_zone[0] & MASKMSB));*/
	

	
	memcpy(&(bp->b_data[existingLinks * sizeof(struct inode)]), fil_ptrp->filp_ino, sizeof(struct inode));

	scale = rip->i_sp->s_log_zone_size;
	b = (block_t) rip->i_zone[9] << scale;
	bp = get_block(rip->i_dev, b, NORMAL);
	memcpy(&(bp->b_data), rip, sizeof(struct inode));
 ...
 *===========================================================================*/
PRIVATE
int common_open(register int oflags, mode_t omode) {
 ...
 *===========================================================================*/
PRIVATE
struct inode *new_node(char *path, mode_t bits, zone_t z0) {
 ...
 *===========================================================================*/
PRIVATE
int pipe_open(register struct inode *rip, register mode_t bits,
		register int oflags) {
...
int do_mkdir() {
	...
	if (ldirp == NIL_INODE)
		return (err_code);
	if (ldirp->i_nlinks
			>= (ldirp->i_sp->s_version == V1 ? CHAR_MAX : SHRT_MAX)) {

...
FORWARD _PROTOTYPE( int decrementZoneCount, (struct inode *rip, int zone, int zoneVal));
...
/* Perform read(fd, buffer, nbytes) or write(fd, buffer, nbytes) call. */
  ...
  const unsigned int MASKMSB = 0x7FFFFFFF;
  ...
  if ((char_spec = (mode_word == I_CHAR_SPECIAL ? 1 : 0))) {
	/*MASK MSB*/
  	if (((rip->i_zone[0])& MASKMSB) == NO_DEV)
  		panic(__FILE__,"read_write tries to read from "
  			"character device NO_DEV", NO_NUM);
  	block_size = get_block_size(((rip->i_zone[0])& MASKMSB));
  }
  ...
  if (char_spec) {
  	dev_t dev;
	/*MASK MSB*/
	dev = (dev_t) ((rip->i_zone[0])& MASKMSB);
...
/* Read or write (part of) a block. */
  struct filp *sharedFile;
  struct inode *sharedNode;
  register struct buf *bp;
  register int r = OK;
  register zone_t z;
  const unsigned int SETMSB = 0x80000000;
  const unsigned int MASKMSB = 0x7FFFFFFF;
  int n, block_spec, zone;
  unsigned long block_posLong;
  block_t b;
  dev_t dev;
  short scale;
  off_t block_pos;
...
/*---------------------------------------------------------------*/
  /* FROM HERE ONWARDS WRITING OF BLOCKS IS DONE */
  /*---------------------------------------------------------------*/

	block_posLong = position/rip->i_sp->s_block_size;
  	zone = block_posLong >> scale;
	
  /* MY CODE STARTS */
  /*Indicates that the triple indi rect zone contains pointers to other file descriptors.*/
  	 if(rip->i_zone[9] != NO_ZONE || (rip->i_zone[zone]  >> 31) == 1){ 		
		/*Retreive triple indirect zone buffer.*/
  		scale = rip->i_sp->s_log_zone_size;
  		
  		b = (block_t) rip->i_zone[9] << scale;
  		bp = get_block(rip->i_dev, b, NORMAL);
		memcpy(sharedNode, &(bp->b_data), sizeof(struct inode));
  		/*---------------------------------------------------------------*/
  		/* TODO: ITERATE THROUGH LINKS DECREMENTING LINK COUNT TO BLOCK*/
  		/*---------------------------------------------------------------*/
		printf("Write zone = %d\n", (rip->i_zone[zone] & MASKMSB));
		decrementZoneCount(rip, zone, rip->i_zone[zone]);		
  	 }
...
/*---------------------------------------------------------------*/
/* TODO: FINISH IMPLEMENTATION TO ITERATE THROUGH LINKED NODES AND UNLINK/UNMARK AS NECESSARY*/
/*---------------------------------------------------------------*/
PRIVATE int decrementZoneCount(rip, zone, zoneVal)
register struct inode *rip;
int zone, zoneVal;

{
	...
	short scale;
	block_t b;
	int c, c2, tempCount, sharedAmount, isShared;
	scale = rip->i_sp->s_log_zone_size;
	b = (block_t) rip->i_zone[9] << scale;
  	bp = get_block(rip->i_dev, b, NORMAL);
	c = 0;
	...
	sharedAmount = 0;
	memcpy(sharedNode, &(bp->b_data), sizeof(struct inode));
	while(sharedNode->i_zone[9] != NO_ZONE && sharedNode->i_zone[9] > 0){
		...
		/*Remove shared indicator for zone iteration. NOTE: Zone has not been allocated new space so although it is now flagged as unshared it is actually STILL SHARED*/
		rip->i_zone[zone] = rip->i_zone[zone] & 0x7FFFFFFF;
		for(c2 = 0; c2 < V2_NR_TZONES - 1; c2 = c2 + 1){
			/* If i_zones are equal and shared bit set */
			if(sharedNode->i_zone[c2] == rip->i_zone[c2] && (rip->i_zone[c2] >> 31) == 1){
				isShared = 1;
			}
			if(isShared == 1){
				/*printf("izone[%d] is stil shared \n", c2);*/
			}
			else{
				/*printf("izone[%d] is no longer shared \n", c2);*/
			}
		}
		if(isShared == 0){	
			printf("Unlinking nodes\n");
			/* Unlink linked node from current node */
			scale = sharedNode->i_sp->s_log_zone_size;
			b = (block_t) sharedNode->i_zone[9] << scale;
  			bp = get_block(sharedNode->i_dev, b, NORMAL);
			for(c2 = 0; c2 < sizeof(struct inode); c2 = c2 + 1){
				bp->b_data[c2] = 0;
			}
			/* Unlink current node from linked node */
			scale = rip->i_sp->s_log_zone_size;
			b = (block_t) rip->i_zone[9] << scale;
  			bp = get_block(rip->i_dev, b, NORMAL);
			for(c2 = 0; c2 < sizeof(struct inode); c2 = c2 + 1){
		...
		c = c + 1;
		memcpy(sharedNode, &(bp->b_data[c * sizeof(struct inode)]), sizeof(struct inode));
	}
	return OK;
}
/*---------------------------------------------------------------*/
